...
def clean_text_v1(text):
    """
    Clean text for scientific summarization:
    - Remove arXiv/LaTeX markers (@xmath, @xcite, \cite, $...$ etc.)
    - Remove references [12], [f7]
    - Remove figure mentions
    - Remove empty brackets and extra spaces
    - (Optional) Keep capitalization & punctuation unless you want to drop it!
    """
    # Remove arXiv/LaTeX
    text = re.sub(r'@xmath\d+', '', text)
    text = re.sub(r'@xcite', '', text)
...
def split_sections(text):
    ...
    for line in lines:
        ...
        # Identify section headers: short, likely a title, or all-caps
    ...
    for i, (section, start_idx) in enumerate(sorted_sections):
        ...
        content = "\n".join(lines_in_section[1:]).strip()  # Remove the section title itself
...
# Optional: add prompt builder utility (for consistent prompt formatting)
